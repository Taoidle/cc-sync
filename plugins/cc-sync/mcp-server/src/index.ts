...
// Sync mode types
export type SyncMode = "basic" | "full" | "custom";
...
interface ConfigItem {
  ...
  type: "file" | "dir";
  optional?: boolean;
  useLFS?: boolean; // Use Git LFS for this item
}

// Basic configuration files/directories to sync (configs only)
const BASIC_CONFIG_ITEMS: ConfigItem[] = [
...
const FULL_CONFIG_ITEMS: ConfigItem[] = [
  ...BASIC_CONFIG_ITEMS,
...
const DEFAULT_CUSTOM_ITEMS: ConfigItem[] = [
  ...BASIC_CONFIG_ITEMS,
  { path: ".claude/plugins", type: "dir", optional: true },
  { path: ".claude/agents", type: "dir", optional: true },
  { path: ".claude/skills", type: "dir", optional: true },
...
interface SyncConfig {
  ...
  initialized: boolean;
  syncMode: SyncMode;
  customItems?: ConfigItem[]; // For custom mode
  useLFS?: boolean; // Whether to use Git LFS
}
 ...
 */
function getConfigItems(mode: SyncMode, customItems?: ConfigItem[]): ConfigItem[] {
  switch (mode) {
    case "basic":
 ...
 */
function checkLFSAvailable(): boolean {
  try {
 ...
 */
function setupLFS(repoPath: string, items: ConfigItem[]): void {
 ...
 * Set sync mode
 */
function setSyncMode(mode: SyncMode, useLFS?: boolean): { success: boolean; message: string } {
 ...
 */
function getSyncModeInfo(): { success: boolean; message: string; mode?: SyncMode } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
 ...
 */
function addCustomItem(item: ConfigItem): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
 ...
 */
function removeCustomItem(path: string): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
 ...
 */
function listCustomItems(): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
 ...
 */
function pushConfigs(options?: { message?: string; mode?: SyncMode }): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
        ...
        if (item.type === "file") {
          ...
          try {
            ...
            if (!item.optional) {
              return {
                success: false,
                message: `Failed to copy ${item.path}: ${(e as Error).message}`,
              };
            }
          }
        }
 ...
 */
function pullConfigs(options?: { mode?: SyncMode }): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        message: `Failed to pull from remote: ${(error as Error).message}\n\nCheck your network connection and remote configuration.`,
      };
    }
 ...
 * Get sync status
 */
function getSyncStatus(): { success: boolean; message: string } {
  const config = getSyncConfig();
  if (!config || !config.initialized) {
    return {
      success: false,
    ...
    tools: [
      {
        name: "init",
        description:
          "Initialize the sync repository. Creates a git repository to store Claude Code configurations.",
        inputSchema: {
          type: "object",
          properties: {
            repoPath: {
      ...
      },
      {
        name: "push",
        description:
          "Push Claude Code configurations to the sync repository. Copies configs, commits, and pushes to remote.",
        inputSchema: {
          type: "object",
          properties: {
            message: {
      ...
      },
      {
        name: "pull",
        description:
          "Pull Claude Code configurations from the sync repository. Fetches from remote and copies to config directory.",
        inputSchema: {
          type: "object",
          properties: {
            mode: {
...
server.setRequestHandler(CallToolRequestSchema, async (request): Promise<any> => {
  ...
  switch (name) {
    case "init":
      ...
      const initMode = (args as { mode?: SyncMode }).mode as SyncMode | undefined;
      const initUseLFS = (args as { useLFS?: boolean }).useLFS;
      if (!repoPath) {
        return {
          content: [
            {
    ...
    case "push":
      return pushConfigs(args as { message?: string; mode?: SyncMode });

    case "pull":
      return pullConfigs(args as { mode?: SyncMode });

    case "status":
      return getSyncStatus();

    case "setMode":
      const mode = (args as { mode: SyncMode }).mode;
      const setModeUseLFS = (args as { useLFS?: boolean }).useLFS;
    ...
    case "addCustomItem":
      return addCustomItem(args as unknown as ConfigItem);

    case "removeCustomItem":
      const removePath = (args as { path: string }).path;
      if (!removePath) {
        return {
          content: [
            {
